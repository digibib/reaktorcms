...
abstract class BasesfRating extends BaseObject  implements Persistent {
	...
	public function getRatedAt($format = 'Y-m-d H:i:s')
	{

		if ($this->rated_at === null || $this->rated_at === '') {
			return null;
		} elseif (!is_int($this->rated_at)) {
						$ts = strtotime($this->rated_at);
			if ($ts === -1 || $ts === false) { 				throw new PropelException("Unable to parse value of [rated_at] as date/time value: " . var_export($this->rated_at, true));
			}
		} else {
			$ts = $this->rated_at;
		}
	...
	public function setRatedAt($v)
	{

		if ($v !== null && !is_int($v)) {
			$ts = strtotime($v);
			if ($ts === -1 || $ts === false) { 				throw new PropelException("Unable to parse date/time value for [rated_at] from input: " . var_export($v, true));
			}
		} else {
			$ts = $v;
		}
		if ($this->rated_at !== $ts) {
			$this->rated_at = $ts;
			$this->modifiedColumns[] = sfRatingPeer::RATED_AT;
		}
	...
	public function hydrate(ResultSet $rs, $startcol = 1)
	{
		try {
			...
			$this->rated_at = $rs->getTimestamp($startcol + 5, null);
	...
	public function fromArray($arr, $keyType = BasePeer::TYPE_PHPNAME)
	{
		...
		if (array_key_exists($keys[5], $arr)) $this->setRatedAt($arr[$keys[5]]);
	}
	...
	public function buildCriteria()
	{
		...
		if ($this->isColumnModified(sfRatingPeer::RATED_AT)) $criteria->add(sfRatingPeer::RATED_AT, $this->rated_at);
	...
	public function copyInto($copyObj, $deepCopy = false)
	{
		...
		$copyObj->setRatedAt($this->rated_at);
