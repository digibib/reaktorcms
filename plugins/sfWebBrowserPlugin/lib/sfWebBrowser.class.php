...
class sfWebBrowser
{
  ...
  public function call($uri, $method = 'GET', $parameters = array(), $headers = array(), $changeStack = true)
  {
    ...
    if(!isset($urlInfo['host']) && isset($this->urlInfo['scheme']) && isset($this->urlInfo['host']))
    {
      // relative link
      $uri = $this->urlInfo['scheme'].'://'.$this->urlInfo['host'].':'.$this->urlInfo['port'].'/'.$uri;
    }
    else if(isset($urlInfo['scheme']) && $urlInfo['scheme'] != 'http' && $urlInfo['scheme'] != 'https')
    {
    ...
    return $this->adapter->call($this, $uri, $method, $parameters, $headers);
  }
