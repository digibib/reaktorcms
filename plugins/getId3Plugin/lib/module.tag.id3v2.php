...
// +----------------------------------------------------------------------+
//
// $Id: module.tag.id3v2.php,v 1.5 2006/06/24 22:58:31 ah Exp $
...
class getid3_id3v2 extends getid3_handler
{
    ...
    public function Analyze() {
        ...
        //    Extended Header
        if (isset($info_id3v2_flags['exthead']) && $info_id3v2_flags['exthead']) {
            ...
            $ext_header = fread ($getid3->fp, 4);
            $info_id3v2['extheaderlength'] = getid3_lib::BigEndianSyncSafe2Int($ext_header);
            ...
            $ext_header_flag_bytes = fread ($getid3->fp, 1);
            $ext_headerflags     = fread ($getid3->fp, $ext_header_flag_bytes);
            $id3_exthead_flags = getid3_lib::BigEndian2Bin($header[5]);
            $info_id3v2['exthead_flags']['update'] = $id3_exthead_flags[1];
            $info_id3v2['exthead_flags']['CRC']    = $id3_exthead_flags[2];
            
            if ($info_id3v2['exthead_flags']['CRC']) {
                $ext_header_raw_crc = fread ($getid3->fp, 5);
                $info_id3v2['exthead_flags']['CRC'] = getid3_lib::BigEndianSyncSafe2Int($ext_header_raw_crc);
            }
            
            $info_id3v2['exthead_flags']['restrictions'] = substr($id3_exthead_flags, 3, 1);
            if ($info_id3v2['exthead_flags']['restrictions']) {
                
                // Restrictions           %ppqrrstt
                $ext_headerrawrestrictions = fread ($getid3->fp, 1);
                $info_id3v2['exthead_flags']['restrictions_tagsize']  = (bindec('11000000') & ord($ext_headerrawrestrictions)) >> 6; // p - Tag size restrictions
                $info_id3v2['exthead_flags']['restrictions_textenc']  = (bindec('00100000') & ord($ext_headerrawrestrictions)) >> 5; // q - Text encoding restrictions
                $info_id3v2['exthead_flags']['restrictions_textsize'] = (bindec('00011000') & ord($ext_headerrawrestrictions)) >> 3; // r - Text fields size restrictions
                $info_id3v2['exthead_flags']['restrictions_imgenc']   = (bindec('00000100') & ord($ext_headerrawrestrictions)) >> 2; // s - Image encoding restrictions
                $info_id3v2['exthead_flags']['restrictions_imgsize']  = (bindec('00000011') & ord($ext_headerrawrestrictions)) >> 0; // t - Image size restrictions
            }
        } // end extended header
        ...
        if (isset($info_id3v2['extheaderlength'])) {
            $size_of_frames -= $info_id3v2['extheaderlength'];
        }
