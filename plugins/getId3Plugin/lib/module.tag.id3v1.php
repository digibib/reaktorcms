...
// +----------------------------------------------------------------------+
//
// $Id: module.tag.id3v1.php,v 1.3 2006/06/13 08:44:50 ah Exp $
...
class getid3_id3v1 extends getid3_handler
{
    ...
    public function Analyze() {
        ...
        if (substr($id3v1_tag, 0, 3) == 'TAG') {
            ...
            $good_format_id3v1_tag = getid3_id3v1::GenerateID3v1Tag($info_id3v1['title'], $info_id3v1['artist'], $info_id3v1['album'], $info_id3v1['year'], getid3_id3v1::LookupGenreID(@$info_id3v1['genre']), $info_id3v1['comment'], @$info_id3v1['track']);
            if ($id3v1_tag !== $good_format_id3v1_tag) {
                $getid3->warning('Some ID3v1 fields do not use NULL characters for padding');
            }
            
            $info_id3v1['tag_offset_end']   = $getid3->info['filesize'];
    ...
    public static function GenerateID3v1Tag($title, $artist, $album, $year, $genre_id, $comment, $track='') {
    
        $id3v1_tag  = 'TAG';
        $id3v1_tag .= str_pad(trim(substr($title,  0, 30)), 30, "\x00", STR_PAD_RIGHT);
        $id3v1_tag .= str_pad(trim(substr($artist, 0, 30)), 30, "\x00", STR_PAD_RIGHT);
        $id3v1_tag .= str_pad(trim(substr($album,  0, 30)), 30, "\x00", STR_PAD_RIGHT);
        $id3v1_tag .= str_pad(trim(substr($year,   0,  4)),  4, "\x00", STR_PAD_LEFT);
        
        if (!empty($track) && ($track > 0) && ($track <= 255)) {
            $id3v1_tag .= str_pad(trim(substr($comment, 0, 28)), 28, "\x00", STR_PAD_RIGHT);
            $id3v1_tag .= "\x00";
            ...
            $id3v1_tag .= str_pad(trim(substr($comment, 0, 30)), 30, "\x00", STR_PAD_RIGHT);
        }
        if (($genre_id < 0) || ($genre_id > 147)) {
            $genre_id = 255; // 'unknown' genre
        }
        switch (gettype($genre_id)) {
            case 'string':
            case 'integer':
                $id3v1_tag .= chr(intval($genre_id));
                break;
            default:
                $id3v1_tag .= chr(255); // 'unknown' genre
                break;
        }
