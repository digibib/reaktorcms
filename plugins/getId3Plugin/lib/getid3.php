...
// +----------------------------------------------------------------------+
//
// $Id: getid3.php,v 1.5 2006/06/24 22:58:30 ah Exp $
...
class getid3
{
    ...
    public $option_md5_data_source   = false;             // Use MD5 of source file if availble - only FLAC, MAC, OptimFROG and Wavpack4.
    public $option_sha1_data         = false;             // Get SHA1 sum of data part - slow.
    
    // Needed for windows only - no spaces allowed
    public $option_helperapps_dir;

    
    // Public variables
    public $windowed;                                     // Bool - Running under MS Windows.
    public $filename;                                     // Filename of file being analysed.
    public $fp;                                           // Filepointer to file being analysed.
    public $info;                                         // Result array.
    
    // Private and protected variables
    private $include_path;                                // getid3 include path.
    private $warnings = array ();
    private $iconv_present;
    ...
    public function __construct()    {
        ...
        $this->windowed = $windowed = strtoupper(substr(PHP_OS, 0, 3)) == 'WIN';
        ...
        // Windows only: - ignore if safe mode on
        // Define locations of helper applications for shorten, sVorbisComment, MetaFLAC as well as other helper functions such as head, tail, md5sum, etc
        // IMPORTANT: This path cannot have spaces in it. If neccesary, use the 8dot3 equivalent ie for "C:/Program Files/Apache/" put "C:/PROGRA~1/APACHE/"
        elseif ($this->windowed) {

            $this->option_helperapps_dir = realpath($this->option_helperapps_dir);
            
            if (!$this->option_helperapps_dir) {
                $this->warning('Running windows and option_helperapps_dir not set to valid directory. Shorten support disabled, md5data/sha1data for ogg vorbis disabled, ogg vorbis/flac tag writing disabled.');
            }
            
            elseif (strstr(realpath($this->option_helperapps_dir), ' ')) {
                $this->warning('option_helperapps_dir contains spaces - not allowed. Shorten support disabled, md5data/sha1data for ogg vorbis disabled, ogg vorbis/flac tag writing disabled.');
            }
        }
    ...
    public function Analyze($filename) {
        ...
        if ($this->option_md5_data || $this->option_sha1_data) {                
            ...
            if ($this->option_md5_data) {
                ...
                // do not call md5_data if md5_data_source is present - set by FLAC, MAC, OptimFROG, Wavpack4 - future MPC/SV8 too...
        ...
        foreach ($tags as $comment_name => $tag_name_encoding_array) {
            ...
            if (@$this->info[$comment_name]['comments']) {
                ...
                if ($this->option_tags_html) {
                    foreach ($this->info['tags'][$tag_name] as $tag_key => $value_array) {
                        foreach ($value_array as $key => $value) {
                            if (is_string($value)) {
                                ...
                                switch ($encoding) {
                                    ...
                                    case 'EUC-JP':
                                    case 'EUCJP':
                                        $html = htmlentities($value, ENT_NOQUOTES, $encoding);
                                        break;
                                    ...
                                    case 'UTF-16BE':
                                    case 'UTF-16':
                                        $html = htmlentities($this->iconv($encoding, 'ISO-8859-1', $value), ENT_NOQUOTES);
                                        break;
                        
                                    default:
                                        throw new getid3_exception('Character set "'.$charset.'" not supported by htmlentities().');
                                }
                                                                
                                $this->info['tags_html'][$tag_name][$tag_key][$key] = str_replace('&#0;', '', $html);
                                unset($html); 
